...
import Quickshell.Services.DesktopEntries  // Necesario para DesktopEntries.byId
...
PopupWindow {
    ...
    // Fallback para Firefox: buscar en metadata MPRIS
    ...
    // ── Firefox bug fixes + retención de carátula ────────────
    readonly property string currentTrackId: player?.metadata?.["mpris:trackid"] ?? ""
    property string displayArtUrl: ""       // La que realmente se muestra
    property string previousTrackId: ""

    // Al cambiar la URL real, decidimos si actualizar o mantener
    onActualArtUrlChanged: {
        if (actualArtUrl !== "") {
            // Nueva imagen válida → la mostramos inmediatamente
            displayArtUrl = actualArtUrl;
        } else if (currentTrackId !== previousTrackId) {
            // La canción cambió y no hay imagen → limpiamos
            displayArtUrl = "";
        }
        // Si la canción es la misma y la URL se volvió vacía, mantenemos la anterior.
        previousTrackId = currentTrackId;
    }

    Component.onCompleted: previousTrackId = currentTrackId
    ...
    // Timer más rápido para la barra de progreso (500 ms)
    Timer {
    ...
    Item {
        ...
        Rectangle {
            ...
            // Fondo blureado con la carátula retenida
            Item {
                anchors.fill: parent
            ...
            Column {
                ...
                // ── Carátula principal con animación slide ─────
                Item {
                    width: parent.width
                    height: parent.width
                    clip: true
                    ...
                    Image {
                        id: artIn
                        width: parent.width; height: parent.height
                        source: popup.displayArtUrl   // ← Usa la imagen retenida
                        fillMode: Image.PreserveAspectFit
