...
PopupWindow {
    ...
    // ── Identificador de pista para controlar cambios ────────
    readonly property string currentTrackId: player?.metadata?.["mpris:trackid"] ?? ""

    // ── Caché en memoria: guardamos el resultado de grabToImage ──
    property var cachedGrab: null

    // URL que realmente se muestra (la de la captura, si existe)
    readonly property string displayArtUrl: cachedGrab ? cachedGrab.url : ""

    // Cuando cambia la pista, olvidamos la captura anterior
    onCurrentTrackIdChanged: {
        cachedGrab = null
        // Forzamos la carga de la nueva imagen (el binding de artIn se encargará)
    }
    ...
    Item {
        ...
        Rectangle {
            ...
            // Fondo blureado (usa displayArtUrl)
            Item {
                anchors.fill: parent
            ...
            Column {
                ...
                // ── Carátula principal con captura en memoria ──
                Item {
                    width: parent.width
                    height: parent.width
                    clip: true
                    ...
                    Image {
                        id: artIn
                        width: parent.width; height: parent.height
                        // La imagen mostrada es siempre la capturada (displayArtUrl), no la original
                        source: popup.displayArtUrl
                        fillMode: Image.PreserveAspectFit
                ...
                // ── Imagen oculta para cargar y capturar ──────
                Image {
                    id: coverLoader
                    visible: false
                    cache: false
                    ...
                    onStatusChanged: {
                        if (status === Image.Ready && source.toString() !== "") {
                            // Capturamos la imagen cargada y guardamos el resultado
                            coverLoader.grabToImage(function(result) {
                ...
                // Actualizamos el coverLoader cuando hay una nueva URL real
                Connections {
                    target: popup
                    function onActualArtUrlChanged() {
                        if (popup.actualArtUrl !== "" && popup.currentTrackId !== "") {
                            coverLoader.source = popup.actualArtUrl
                        }
                    }
                    function onCurrentTrackIdChanged() {
                        coverLoader.source = ""  // Limpia el loader al cambiar de pista
