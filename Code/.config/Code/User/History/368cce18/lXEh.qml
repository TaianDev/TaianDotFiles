...
PopupWindow {
    ...
    // ════════════════════════════════════════════════════════════
    //  Carátula estable (soluciona el bug de Firefox sin inventos)
    // ════════════════════════════════════════════════════════════
    // URL real después de buscar en metadata
    ...
    // Mantiene la última carátula válida hasta que cambie la canción
    property string stableArtUrl: ""
    property string lastTrackId: ""

    onActualArtUrlChanged: {
        if (actualArtUrl !== "") {
            // Nueva imagen válida → actualizar
            stableArtUrl = actualArtUrl
        } else if (trackId !== lastTrackId) {
            // Canción distinta y no hay imagen → borrar
            stableArtUrl = ""
        }
        // Si la canción es la misma, conservamos la imagen anterior (Firefox la borra)
        lastTrackId = trackId
    }

    onTrackIdChanged: {
        if (trackId !== lastTrackId) {
            // Al cambiar de pista, si ya no hay URL, limpiamos
    ...
    Component.onCompleted: {
        if (actualArtUrl !== "") stableArtUrl = actualArtUrl
        lastTrackId = trackId
    }
    ...
    // ── Botón SVG (optimizado con cache) ─────────────────────
            ...
            Column {
                ...
                Item {
                    ...
                    Image {
                        id: artIn
                        width: parent.width; height: parent.height
                        source: popup.stableArtUrl   // Usa la URL estable
                        fillMode: Image.PreserveAspectFit
