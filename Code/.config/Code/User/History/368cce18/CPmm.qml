...
PopupWindow {
    ...
    // ════════════════════════════════════════════════════════════
    property var cachedGrab: null
    property string cachedTrackId: ""               // ← Pista asociada a la caché
    ...
    function refreshArtCache() {
        // Si la pista NO ha cambiado y ya tenemos una imagen en caché, la conservamos.
        // (Firefox puede mandar artUrl vacío tras el blob, así que lo ignoramos mientras
        //  el trackId siga siendo el mismo.)
        if (trackId !== "" && trackId === cachedTrackId && cachedGrab && cachedGrab.url !== "") {
            return
        }

        // La pista cambió o no hay caché → limpiamos y cargamos la nueva imagen (si existe)
        cachedGrab = null
        ...
        onStatusChanged: {
            if (status === Image.Ready && source.toString() !== "") {
                grabToImage(function(result) {
                    if (result && popup.trackId === coverLoader._loadedTrackId) {
                        popup.cachedGrab = result
                        popup.cachedTrackId = coverLoader._loadedTrackId
                    }
                })
            }
        }
    ...
    // ════════════════════════════════════════════════════════════
    //  Iconos de reproductor (sin cambios respecto a tu original)
    // ════════════════════════════════════════════════════════════
    ...
    // ══════════════════════════════════════════════════════════
    // BURBUJA (sin cambios en animación ni estructura)
    // ══════════════════════════════════════════════════════════
    Item {
        id: bubbleRoot
        anchors.fill: parent
        ...
        Rectangle {
            ...
            Item {
                ...
                Image {
                    ...
                    cache: false
                    asynchronous: true
                    sourceSize.width: 300   // ← tamaño original, lo dejamos como estaba
                    sourceSize.height: 300
                }
                FastBlur {
            ...
            Column {
                ...
                // ── Carátula principal (animación de transición sin cambios) ──
                Item {
                    width: parent.width
                    height: parent.width
                    clip: true
