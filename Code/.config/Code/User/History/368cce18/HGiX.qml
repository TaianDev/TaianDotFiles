...
PopupWindow {
    ...
    // Substituciones conocidas: playerName -> nombre de icono real
    ...
    function playerIcon(mprisPlayer) {
        ...
        // 1. Substitucion directa por playerName (brave -> brave-browser)
        ...
        // 5. identity lowercase (ej: "Firefox" -> "firefox")
        if (identity !== "") {
            const id = identity.toLowerCase()
            if (iconExists(id)) return Quickshell.iconPath(id, "")
        }

        // Sin icono — mostrara la inicial
        return ""
    }
