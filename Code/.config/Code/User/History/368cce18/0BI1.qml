...
PopupWindow {
    ...
    // ── Identificador único de pista ────────────────────────
    readonly property string trackId: player?.metadata?.["mpris:trackid"] ?? ""

// ── Identidad del reproductor activo ───────────────────────
// Puede ser playerName, desktopEntry o una combinación
readonly property string playerIdentity: {
    if (!player) return ""
    return player.desktopEntry || player.playerName || ""
}

// ════════════════════════════════════════════════════════════
//  Carátula estable (sin borrados al cambiar de reproductor)
// ════════════════════════════════════════════════════════════
...
property string lastPlayerIdentity: ""   // ← quién puso la última carátula

onActualArtUrlChanged: {
    if (actualArtUrl !== "") {
        // Nueva imagen válida → actualizamos y recordamos quién la puso
        stableArtUrl = actualArtUrl
        lastPlayerIdentity = playerIdentity
    } else if (trackId !== lastTrackId && playerIdentity === lastPlayerIdentity) {
        // Mismo reproductor, distinta canción y sin imagen → borrar
        stableArtUrl = ""
    }
    lastTrackId = trackId
}
...
onTrackIdChanged: {
    // Solo borramos si sigue siendo el mismo reproductor que puso la última imagen
    if (trackId !== lastTrackId && playerIdentity === lastPlayerIdentity) {
        if (actualArtUrl === "") stableArtUrl = ""
    }
    lastTrackId = trackId
}
...
Component.onCompleted: {
    ...
    Item {
        ...
        Rectangle {
            ...
            // Fondo blureado (usa la imagen estable)
            Item {
                anchors.fill: parent
                visible: popup.stableArtUrl !== ""

                Image {
                    id: popupBg
                    anchors.centerIn: parent
                    source: popup.stableArtUrl
            ...
            Column {
                ...
                Item {
                    ...
                    Rectangle {
                        ...
                        visible: popup.stableArtUrl === "" || artIn.status === Image.Error
