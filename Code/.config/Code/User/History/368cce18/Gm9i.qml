...
PopupWindow {
    ...
    // ════════════════════════════════════════════════════════════
    // Cache de carátulas POR PLAYER Y PISTA
    // Estructura: { "playerName|trackId": "url" }
    // ════════════════════════════════════════════════════════════
    property var artCache: ({})

    // Clave única para el player+pista actual
    readonly property string cacheKey: {
        const pname = player?.playerName ?? player?.identity ?? "unknown"
        return pname + "|" + trackId
    }

    // URL estable: lo que hay en caché para el player actual
    readonly property string stableArtUrl: artCache[cacheKey] ?? ""

    // Cuando cambia la artUrl real, guardarla en caché bajo la clave actual
    // SOLO si la clave ya pertenece al reproductor activo (evita sobreescribir
    // la entrada del reproductor anterior durante la transición).
    onArtUrlChanged: {
        if (artUrl === "") return

        const pname = player?.playerName ?? player?.identity ?? "unknown"
        if (cacheKey.startsWith(pname + "|")) {
    ...
    // Cuando cambia el player o la pista, copiamos la artUrl actual a la nueva clave
    // para que la carátula se muestre de inmediato si ya está disponible.
    onCacheKeyChanged: {
        if (artUrl !== "" && cacheKey !== "|") {
            const updated = Object.assign({}, artCache)
            updated[cacheKey] = artUrl
