...
import Qt.labs.platform          // Necesario para StandardPaths
import Quickshell
...
PopupWindow {
    ...
    readonly property string actualArtUrl: {
    ...
    // ── Sistema de caché para la carátula ────────────────────
    readonly property string currentTrackId: player?.metadata?.["mpris:trackid"] ?? "unknown"
    readonly property string cacheDir: {
        var path = StandardPaths.writableLocation(StandardPaths.CacheLocation)
        return "file://" + path + "/quickshell-musicpopup/"
    }

    // Archivo local esperado para la pista actual
    function cachedArtPath(trackId) {
        return cacheDir + trackId.replace(/[^a-zA-Z0-9]/g, '_') + ".png"
    }

    property string displayArtUrl: ""   // lo que realmente se muestra

    // Imagen oculta para cargar, capturar y guardar en caché
    Image {
        id: coverLoader
        visible: false
        cache: false
        ...
        onStatusChanged: {
            if (status === Image.Ready && source.toString() !== "") {
                grabToImage(function(result) {
                    if (result && popup.currentTrackId === coverLoader._trackId) {
                        var filePath = popup.cachedArtPath(coverLoader._trackId)
                        result.saveToFile(filePath)
                        // Forzamos la recarga añadiendo un timestamp como parámetro
                        popup.displayArtUrl = filePath + "?t=" + Date.now()
                    }
                })
            }
        }
    ...
    // Cargamos la imagen desde el caché (puede no existir aún) o iniciamos descarga
    onCurrentTrackIdChanged: {
        var path = popup.cachedArtPath(currentTrackId)
        // Mostramos el archivo local (si existe se verá, si no, placeholder)
        popup.displayArtUrl = path
        if (popup.actualArtUrl !== "") {
            // Lanzamos carga y posterior guardado
            coverLoader._trackId = currentTrackId
            coverLoader.source = popup.actualArtUrl
        }
    }

    onActualArtUrlChanged: {
        if (actualArtUrl !== "" && popup.currentTrackId !== "") {
            coverLoader._trackId = popup.currentTrackId
            coverLoader.source = actualArtUrl
        }
    }
    ...
    Component.onCompleted: {
        if (currentTrackId !== "") {
            displayArtUrl = cachedArtPath(currentTrackId)
            if (actualArtUrl !== "") {
                coverLoader._trackId = currentTrackId
                coverLoader.source = actualArtUrl
            }
    ...
    Item {
        ...
        Rectangle {
            ...
            // Fondo con el arte local en caché
            Item {
                anchors.fill: parent
                ...
                Image {
                    ...
                    cache: false           // ya está en disco, no hace falta cache
                    asynchronous: true
            ...
            Column {
                ...
                // ── Selector de players (optimizado) ──────────
                Row {
                ...
                // ── Carátula principal (ahora usa displayArtUrl) ──
                Item {
                    width: parent.width
                    height: parent.width
                    clip: true
                    ...
                    Image {
                        id: artIn
                        width: parent.width; height: parent.height
                        source: popup.displayArtUrl   // archivo local
                        fillMode: Image.PreserveAspectFit
