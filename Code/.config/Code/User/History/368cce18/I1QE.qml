...
PopupWindow {
    ...
    // ── Datos originales del reproductor ─────────────────────
    ...
    // ════════════════════════════════════════════════════════════
    //  Caché en memoria de la carátula (solución Firefox + rapidez)
    // ════════════════════════════════════════════════════════════
    property var cachedGrab: null                // Resultado del grab
    readonly property string displayArtUrl: cachedGrab ? cachedGrab.url : ""

    // Se dispara cuando la URL de la carátula o la pista cambian
    onArtUrlChanged: refreshArtCache()
    onTrackIdChanged: refreshArtCache()

    function refreshArtCache() {
        // Si la pista no ha cambiado y ya tenemos caché, no hacemos nada
        if (trackId !== "" && cachedGrab && cachedGrab.url !== "") {
            // Si el arte es el mismo, mantenemos la caché
            // (Firefox puede mandar artUrl vacío tras el blob, así que ignoramos)
            return
        }
        // Si la pista cambió o no hay caché, limpiamos y cargamos de nuevo
        cachedGrab = null
        if (artUrl !== "" && trackId !== "") {
            // Lanzamos la carga en segundo plano
            coverLoader.source = artUrl
        }
    }

    // Imagen oculta que carga y captura la carátula
    Image {
        id: coverLoader
        visible: false
        cache: false
        ...
        onStatusChanged: {
            if (status === Image.Ready && source.toString() !== "") {
                // Capturamos la imagen en memoria
                grabToImage(function(result) {
                    if (result) {
                        // Solo guardamos si la pista sigue siendo la misma
                        if (popup.trackId === coverLoader._loadedTrackId)
                            popup.cachedGrab = result
                    }
                })
            }
        }
        property string _loadedTrackId: ""
        onSourceChanged: _loadedTrackId = popup.trackId
    }
    ...
    // Timer más frecuente para una barra muy fluida (250 ms)
    Timer {
    ...
    // ════════════════════════════════════════════════════════════
    //  Iconos de reproductor (con caché activada)
    // ════════════════════════════════════════════════════════════
    ...
    function playerIcon(mprisPlayer) {
        ...
        if (playerName !== "" && iconSubstitutions[playerName]) {
            const sub = iconSubstitutions[playerName]
            if (iconExists(sub)) return Quickshell.iconPath(sub, "")
        }
    ...
    component SvgBtn: Item {
        ...
        Image {
            ...
            cache: true               // ← Cache activado (instantáneo)
            asynchronous: true
            layer.enabled: true
    ...
    // ══════════════════════════════════════════════════════════
    // BURBUJA (animación de entrada mantenida)
    // ══════════════════════════════════════════════════════════
    Item {
        id: bubbleRoot
        anchors.fill: parent
        ...
        Rectangle {
            ...
            // Fondo blureado (usa la copia cacheada, carga instantánea)
            Item {
                anchors.fill: parent
                ...
                Image {
                    ...
                    cache: false           // Ya está en memoria, no necesita cache
                    asynchronous: true
                    sourceSize.width: 200  // ← Tamaño reducido para mayor velocidad
                    sourceSize.height: 200
                }
                FastBlur {
                    anchors.fill: popupBg
                    source: popupBg
                    radius: 60             // ← Un poco menor para menos carga
                }
                Rectangle {
                    anchors.fill: parent
            ...
            Column {
                ...
                // ── Selector de players (con iconos cacheados) ─
                Row {
                ...
                // ── Carátula principal (cacheada en memoria) ──
                Item {
                    width: parent.width
                    height: parent.width
                    clip: true
                    ...
                    Image {
                        id: artIn
                        width: parent.width; height: parent.height
                        source: popup.displayArtUrl   // ← Carga instantánea desde memoria
                        fillMode: Image.PreserveAspectFit
                        cache: false                  // No necesario, ya está en RAM
                        asynchronous: true
                        sourceSize.width: 300; sourceSize.height: 300
                        layer.enabled: true
                        layer.effect: OpacityMask {
