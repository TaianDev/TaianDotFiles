...
Timer {
    ...
    onTriggered: {
        // 1. RAM
        try {
            var memInfo = QmlFs.readFile("/proc/meminfo");
        ...
        // 2. CPU
        try {
            var cpuStat = QmlFs.readFile("/proc/stat");
        ...
        // 3. Temperatura
            try {
            // Ajusta la ruta si tu sensor es otro (p.ej. /sys/class/hwmon/hwmon0/temp1_input)
                var tempRaw = QmlFs.readFile("/sys/class/thermal/thermal_zone0/temp");
                if (tempRaw.length > 0)
