...
Rectangle {
    ...
    Timer {
        ...
        onTriggered: {
            ...
            // 3. Temperatura
            try {
                // Verifica que esta ruta exista en tu sistema:
                //   ls /sys/class/thermal/
                // Si no existe, pruébalo con thermal_zone1, thermal_zone2, o hwmon
                var tempRaw = Fs.readFile("/sys/class/thermal/thermal_zone0/temp");
                if (tempRaw.length > 0)
                    root.tempValue = Math.round(parseInt(tempRaw) / 1000);
            } catch(e) {
                console.log("Error temperatura:", e);
            }
        }
    }
}