    ...
    MouseArea {
        ...
        onClicked: (mouse) => {
            ...
            // 🌟 Clic Izquierdo: Uso exclusivo para la Dynamic Island
                ...
                // 1. Si hay notificación de fin de tiempo, la descarta y no hace nada más
                if (root.tmEndedNotification) {
                    root.tmEndedNotification = false
                    return
                }
                
                // 2. Si el Cronómetro es la instancia activa y tiene datos
                if (root.activeInstance === 1 && (root.swElapsedMs > 0 || root.swRunning)) {
                    root.showSwControls = !root.showSwControls
                    root.showTmControls = false
                    autoHideSw.stop()
                } 
                // 3. Si el Temporizador es la instancia activa y está funcionando
                ...
                // 🌟 NOTA: Eliminamos el "else { datePopup.isOpened = !datePopup.isOpened }"
                // Si no se cumple nada de lo anterior (ej. cronómetro detenido a 0), 
                // el clic izquierdo muere aquí y no hace absolutamente nada.
            }
        }
    }
