...
Item {
    ...
    // ── Carátula estable (soluciona el bug de Firefox/Zen) ───
    ...
    onArtUrlChanged: {
        if (artUrl !== "") {
            ...
            // Canción distinta sin imagen → borrar
            stableArtUrl = ""
        }
        // Si la canción no cambió, conservamos la anterior (Firefox la borra)
        lastTrackId = trackId
    }

    onTrackIdChanged: {
        if (trackId !== lastTrackId) {
            // Al cambiar de pista, si no hay imagen limpiamos
            if (artUrl === "") stableArtUrl = ""
            lastTrackId = trackId
        }
    }
