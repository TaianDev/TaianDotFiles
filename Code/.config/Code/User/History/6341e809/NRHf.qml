...
import Qt5Compat.GraphicalEffects // 🌟 Necesario para pintar los SVGs
import Quickshell
...
Item {
    ...
    // ── 1. Escáner Inteligente de Redes ──
    Process {
        id: scanner
        // Simplificamos el comando de bash, el trabajo duro lo hará JavaScript
        command: ["bash", "-c", "nmcli -t -f SSID,SECURITY,ACTIVE,SIGNAL dev wifi | grep -v '^:'"]
        ...
        onStdout: (output) => {
            ...
            let activeNet = null;
            let otherMap = {}; // Usamos un diccionario para borrar duplicados
            ...
            for(let line of lines) {
                ...
                if(parts.length >= 4 && parts[0] !== "") {
                    ...
                    if (isActive) {
                        // Guardamos la red activa por separado
                        activeNet = { ssid: ssid, security: security, active: true, signal: signal };
                    } else {
                        // Guardamos las demás, priorizando las de mejor señal
            ...
            // 🌟 MAGIA: Insertamos la red activa SIEMPRE en la posición 0 (arriba)
            if (activeNet) {
                wifiModel.append(activeNet);
                delete otherMap[activeNet.ssid]; // Evita que se duplique abajo
            }

            // Insertamos el resto de las redes
    ...
    ScrollView {
        ...
        clip: true
        ScrollBar.vertical.policy: ScrollBar.AlwaysOff // 🌟 Oculta la barra de desplazamiento
                ...
                RowLayout {
                    ...
                    Item {
                        ...
                        Image {
                            ...
                            visible: false // Se oculta el original negro
                        }
                        ColorOverlay {
                            anchors.fill: wifiIcn
                            source: wifiIcn
                            // Azul si está conectado, Blanco si no
                    ...
                    // 🌟 Ícono de Candado (Pintado de grisáceo y oculto en la red conectada)
                    Item {
                        ...
                        ColorOverlay {
                            ...
                            source: lockIcn
                            color: Qt.rgba(1, 1, 1, 0.4) // Gris semitransparente
                        }
                    }
                }
