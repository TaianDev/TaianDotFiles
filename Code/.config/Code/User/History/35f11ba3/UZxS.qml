import QtQuick
import QtQuick.Layouts
import Quickshell
import Quickshell.Networking

Item {
    id: root

    property string iconsPath: Qt.resolvedUrl("../../assets/icons/")
    property real rxSpeed: 0
    property real txSpeed: 0

    function formatSpeed(bps) {
        if (bps < 1024)        return Math.round(bps) + "B/s"
        if (bps < 1048576)     return (bps / 1024).toFixed(1) + "K/s"
        return (bps / 1048576).toFixed(1) + "M/s"
    }

    // ── Dispositivo WiFi (reactivo) ──────────────────────────
    readonly property var wifiDevice: {
        const model = Networking.devices  // ListModel reactivo
        for (let i = 0; i < model.count; i++) {
            const dev = model.get(i)
            // Usa el nombre completo de la enumeración
            if (dev.type === NetworkDeviceType.Wifi) {
                console.log("WiFi device found:", dev.interfaceName, "connected:", dev.connected)
                return dev
            }
        }
        console.log("No WiFi device in", model.count, "devices")
        return null
    }

    readonly property bool enabled:   wifiDevice !== null
    readonly property bool connected: wifiDevice?.connected ?? false

    // ── Red activa (primero intenta activeNetwork, luego busca en networks) ──
    readonly property var activeNetwork: {
        if (!wifiDevice) return null
        // Intenta la vía directa
        if (wifiDevice.activeNetwork) {
            console.log("activeNetwork directo:", wifiDevice.activeNetwork.ssid)
            return wifiDevice.activeNetwork
        }
        // Respaldo: iterar networks conectadas
        const nets = wifiDevice.networks
        for (let j = 0; j < nets.count; j++) {
            const net = nets.get(j)
            if (net.connected) {
                console.log("Red conectada encontrada en networks:", net.ssid)
                return net
            }
        }
        console.log("Sin red conectada detectada")
        return null
    }

    readonly property string ssid:     activeNetwork?.ssid     ?? ""
    readonly property int    strength: activeNetwork?.strength ?? 0

    readonly property string icon: {
        if (!enabled)   return iconsPath + "no-wifi.svg"
        if (!connected) return iconsPath + "wifi-none.svg"
        if (strength >= 66) return iconsPath + "wifi-full.svg"
        if (strength >= 33) return iconsPath + "wifi-medium.svg"
        return iconsPath + "wifi-low.svg"
    }

    // Debug extra: reacciona a cambios de ssid
    onSsidChanged: console.log("SSID ahora:", ssid, " strength:", strength)

    implicitWidth:  row.implicitWidth
    implicitHeight: row.implicitHeight
    opacity: enabled ? 1.0 : 0.4
    Behavior on opacity { NumberAnimation { duration: 200 } }

    RowLayout {
        id: row
        spacing: 5

        SvgIcon { source: root.icon; size: 13 }

        ScrollingText {
            text: root.connected ? root.ssid
                : (root.enabled ? "Sin red" : "WiFi Off")
            maxWidth: 60
            fontSize: 11
            color: root.connected ? "#ffffff" : Qt.rgba(1,1,1,0.4)
        }

        RowLayout {
            spacing: 4
            visible: root.connected

            RowLayout {
                spacing: 2
                SvgIcon { source: root.iconsPath + "receive.svg"; size: 10; tint: "#89dceb" }
                Text { text: root.formatSpeed(root.rxSpeed); color: "#89dceb"; font.pixelSize: 10 }
            }
            RowLayout {
                spacing: 2
                SvgIcon { source: root.iconsPath + "transmit.svg"; size: 10; tint: "#a6e3a1" }
                Text { text: root.formatSpeed(root.txSpeed); color: "#a6e3a1"; font.pixelSize: 10 }
            }
        }
    }
}